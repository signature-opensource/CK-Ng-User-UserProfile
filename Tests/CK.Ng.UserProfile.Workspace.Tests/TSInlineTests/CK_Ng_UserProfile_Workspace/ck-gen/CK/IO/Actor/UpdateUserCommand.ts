import { CommandModel, ICommand, IAbstractCommand } from '../../Cris/Model';
import { UpdateUserCommandResult } from './UpdateUserCommandResult';
import { ICommandSimpleBatch } from '../../Cris/ICommandSimpleBatch';
import { ICommandCurrentCulture } from '../../Cris/ICommandCurrentCulture';
import { ICommandAuthNormal } from '../../Auth/ICommandAuthNormal';
import { CTSType } from '../../Core/CTSType';

class MyCommandModel extends CommandModel<UpdateUserCommand> {
protected override doApplyAmbientValues( command: any, a: any, o: any ): void {
if( command.currentCultureName === undefined ) command.currentCultureName = o.currentCultureName !== null ? o.currentCultureName : a.currentCultureName;

if( command.actorId === undefined ) command.actorId = o.actorId !== null ? o.actorId : a.actorId;

}

}
export class UpdateUserCommand implements ICommand<UpdateUserCommandResult|undefined>, ICommandSimpleBatch, ICommandCurrentCulture, ICommandAuthNormal {
public readonly commands: Array<{command: IAbstractCommand, description?: string}>;
/**
 * The current culture name that must be used when handling
 * an event or a command. When null, the currently available CurrentCultureInfo is
 * not changed.
 * (This is a AmbientService Value.)
 **/
public currentCultureName?: string;
/**
 * (This is a AmbientService Value.)
 **/
public actorId?: number;
public constructor()
public constructor(
commands?: Array<{command: IAbstractCommand, description?: string}>,
currentCultureName?: string,
actorId?: number)
constructor(
commands?: Array<{command: IAbstractCommand, description?: string}>,
currentCultureName?: string,
actorId?: number)
{
this.commands = commands ?? [];
this.currentCultureName = currentCultureName;
this.actorId = actorId;
CTSType["CK.IO.Actor.IUpdateUserCommand"].set( this );
}

get commandModel(): CommandModel<this> { return UpdateUserCommand.#m; }

static #m: MyCommandModel = new MyCommandModel();
readonly _brand!: ICommand<UpdateUserCommandResult|undefined>["_brand"] & ICommandSimpleBatch["_brand"] & ICommandCurrentCulture["_brand"] & ICommandAuthNormal["_brand"] & {"38":any};
}
